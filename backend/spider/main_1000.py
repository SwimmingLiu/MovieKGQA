...
def analysisDoubanTop1000part25(html: requests.Response.text):
    """
    一页电影列表 25部
    :param html:
    ...
    movies_info = soup.find(class_='article')
    movies_list = movies_info.findAll('div', class_='doulist-item')
    movies_url = []
    for movie in movies_list:
        a = movie.findNext(class_='post').find_next('a')
        movie_url = a['href']
...
if __name__ == '__main__':
    ...
    # 遍历Top1000的所有页面
    for idx in range(40):
        target_url = f'https://www.douban.com/doulist/109986428/?start={25 * idx}&sort=seq&playable=0&sub_type='
        response = requests.get(url=target_url, headers=headers)
        html = response.text
        if response.status_code == 200:
            ...
            movies_url = analysisDoubanTop1000part25(html)
            ...
            movie_file_name = 'movie_file.csv'
            actor_file_name = 'actor_file.csv'
