...
class QA:
    ...
    def answer(self, sentence: str):
        ...
        try:
            ...
            elif label == 'genres_by_actor':
                return self._get_genres_by_actor(actors[0])
            else:
    ...
    def _get_introduction_by_movie(self, movie_name: str):
        result = self._database.find_movie_img(
            """
            MATCH (m:Movie)
            WHERE m.name = $movie_name
            RETURN m.introduction, m.image
            Limit 1
            """,
    ...
    def _get_release_date_by_movie(self, movie_name: str):
        ...
        if result and type(result) == str:
            return result
        elif result and type(result) == datetime:
            result.strftime("%Y/%m/%d")
        else:
            return QA._NOT_FOUND_MESSAGE
    ...
    def _get_biography_by_actor(self, actor_name: str):
        isalpha = all(ord(c) < 128 for c in actor_name)
        if isalpha:
            result = self._database.find_actor_img(
                """
                MATCH (a:Actor)
                WHERE a.english_name = $actor_name
